        ...
        public virtual void Importar(int ano)
        {
            ...
            using (logger.BeginScope(new Dictionary<string, object> { ["Ano"] = ano }))
            {
                ...
                foreach (var arquivo in arquivos)
                {
                    ...
                    {
                        var arquivoJaProcessado = BaixarArquivo(_urlOrigem, caminhoArquivo);
                        if(anoAtual != ano && importacaoIncremental && arquivoJaProcessado && arquivos.Count == 1 && config.Estado != Estado.Piaui) 
                        {
