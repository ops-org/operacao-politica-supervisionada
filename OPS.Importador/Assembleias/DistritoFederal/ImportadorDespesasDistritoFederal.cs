    ...
    public class ImportadorDespesasDistritoFederal : ImportadorDespesasArquivo
    {
        ...
        public ImportadorDespesasDistritoFederal(IServiceProvider serviceProvider) : base(serviceProvider)
        {
            config = new ImportadorCotaParlamentarBaseConfig()
            {
                ...
                Estado = Core.Enumerators.Estados.DistritoFederal,
        ...
        public override Dictionary<string, string> DefinirUrlOrigemCaminhoDestino(int ano)
        {
            ...
            //    caminhoArquivo = $"{appSettings.TempFolder}/CLDF-{ano}.xlsx";
            ...
            //        caminhoArquivo = $"{appSettings.TempFolder}/CLDF-{ano}-{mes}.xlsx";
            ...
            //    caminhoArquivo = $"{appSettings.TempFolder}/CLDF-{ano}.csv";
            //    arquivos.Add(urlOrigem, caminhoArquivo);
            //}

            caminhoArquivo = Path.Combine(tempFolder, $"CLDF-{ano}.xlsx");
        ...
        private void CarregaDadosCsv(string filename, int ano)
        {
        ...
        private void CarregaDadosXlsx(string filename, int ano)
        {
            string sResumoValores = string.Empty;
                    ...
                    for (int i = 1; i <= worksheet.Dimension.End.Row; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(worksheet.Cells[i, DATA_COMPROVANTE].Value?.ToString()))
                        {
                            if (worksheet.Cells[i, 7].Value is double)
                                ...
                                despesaTemp.DataEmissao = DateOnly.Parse(worksheet.Cells[i, DATA_COMPROVANTE].Value.ToString(), cultureInfo);
                            else
                            {
                                ...
                                despesaTemp.DataEmissao = DateOnly.Parse(data.Replace("31/05/22021", "31/05/2021"));
                            }
