    ...
    public class ImportadorDespesasPiaui : ImportadorDespesasArquivo
    {
        ...
        public override Task Importar(int ano)
        {
            using (logger.BeginScope(new Dictionary<string, object> { ["Ano"] = ano }))
            {
                ...
                var caminhoArquivo = Path.Combine(tempFolder, "CLPI-despesas-legislatura.csv");
                fileManager.ProcessarArquivoTemp(dbContext, caminhoArquivo, config.Estado);
                ...
                using (var reader = new StreamReader(caminhoArquivo, Encoding.GetEncoding("UTF-8")))
                {
                    using (var csv = new CsvReader(reader, CultureInfo.CreateSpecificCulture("pt-BR")))
                    {
                        while (csv.Read())
                        {
                            ...
                            despesaTemp.DataEmissao = DateOnly.Parse("01/" + csv[ColunasDespesas.Competencia.GetHashCode()], cultureInfo);
