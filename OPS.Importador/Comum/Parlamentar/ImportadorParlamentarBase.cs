    ...
    public abstract class ImportadorParlamentarBase : IImportadorParlamentar
    {
        ...
        protected DeputadoEstadual GetDeputadoByNameOrNew(string nomeParlamentar)
        {
            var id = dbContext.DeputadoEstadualDeparas
                .Where(d => d.IdEstado == config.Estado.GetHashCode() && EF.Functions.ILike(PgFunctions.Unaccent(d.Nome), PgFunctions.Unaccent(nomeParlamentar)))
                .Select(d => d.Id)
                .FirstOrDefault();
            ...
            return dbContext.DeputadosEstaduais.Find((int)id);
        }
