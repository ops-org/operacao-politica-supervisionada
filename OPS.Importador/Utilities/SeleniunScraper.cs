...
namespace OPS.Importador.Utilities
{
    public class SeleniunScraper
    {
        ...
        public void BaixarArquivosParana(int ano)
        {
            ...
            try
            {
                ...
                for (int mes = 1; mes <= 12; mes++) // TODO
                {
                    ...
                    SetSelectOptionByText(driver, "mes", ResolveMes(mes));
                    driver.FindElement(OpenQA.Selenium.By.Id("exercicio")).SendKeys(ano.ToString());
                    ...
                    do
                    {
                        ...
                        logger.LogInformation("Consultando ano {Ano} e mês {Mes}", ano, mes);
        ...
        public void BaixarArquivosPiaui()
        {
            ...
            options.AddUserProfilePreference("download.default_directory", "C:\\temp\\Piaui");
            ...
            var file = "C:\\temp\\Piaui\\grid_transp_publico_gecop.csv";
            if (File.Exists(file))
                File.Delete(file);

            wait.Until(d => d.FindElement(By.Id("idBtnDown"))).Click();
            logger.LogInformation("Baixando arquivo");

            wait.Until(d => File.Exists(file));
            logger.LogInformation("Arquivo baixado");
