...
export default {
  ...
  mounted() {
    ...
    axios
      .get(`${process.env.API}/deputado/tipodespesa`)
      .then((response) => {
        this.despesas = response.data;
      });

    axios.get(`${process.env.API}/deputado/pesquisa`).then((response) => {
  ...
  methods: {
    ...
    Detalhar(agrupar) {
      ...
      switch (vm.filtro.agrupar) {
        case '1': // Deputado
          vm.filtro.parlamentar = [this.selectedRow.id_cf_deputado];
          break;
        case '2': // Despesa
