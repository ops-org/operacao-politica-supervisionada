...
app.controller('DeputadoEstadualListaController', ["$rootScope", "$scope", "$tabela", "$api", "$queryString",
    function ($rootScope, $scope, $tabela, $api, $queryString) {
    	...
    	var init = function () {
    		...
    		OPS.select("#lstParlamentar", "./Api/DeputadoEstadual", qs.IdParlamentar);
    		$scope.filtro.IdParlamentar = qs.IdParlamentar;

    		OPS.select("#lstDespesa", "./Api/DeputadoEstadual/TipoDespesa", qs.Despesa);
	        ...
	        $('#lblDeputadoEstadualUltimaAtualizacao').text(window.DeputadoEstadualUltimaAtualizacao);
	    }

        $scope.Pesquisar = function (page_load) {
            var lstParlamentar = $("#lstParlamentar").val();
            if (lstParlamentar && lstParlamentar.length > 50) {
                alert('No maximo 50 deputados podem ser selecionados por vez. Redefina sua busca e tente novamente.');
                return;
	        }
    	...
    	$scope.BuscaGrid = function () {
    		$scope.tableParams = $tabela.databind('DeputadoEstadual/Lancamentos', $scope.filtro);
    	}
