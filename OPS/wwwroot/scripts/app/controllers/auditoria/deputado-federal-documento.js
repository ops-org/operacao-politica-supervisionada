        ...
        $api.get('Deputado/Documento', $routeParams.id).success(function (doc) {
            ...
            if (doc.url_documento) { //NF-e
                if (doc.url_documento.toLowerCase().indexOf('idedocumentofiscal') !== -1) {
                    $scope.documento.url_documento = doc.url_documento.toLowerCase().replace('http://camara.leg.br/', 'https://www.camara.gov.br/').replace('idedocumentofiscal', 'ideDocumentoFiscal');
                } else {
                    $scope.documento.url_documento_nfe = doc.url_documento.toLowerCase();
                }
